...
struct N8nService: NotificationService {
    let id: UUID
    ...
    func send(notification: AppNotification, resolvedFields: [String: String]) async throws {
        ...
        guard let url = URL(string: webhookURL) else {
            throw N8nError.invalidURL(webhookURL)
        }
        ...
        if let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode >= 300 {
            ...
            throw N8nError.httpError(httpResponse.statusCode, snippet)
        }
    }
}
...
enum N8nError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidURL(let url): return "Invalid n8n webhook URL: \(url)"
        case .httpError(let code, let body): return "n8n returned \(code): \(body)"
        }
    }
}
